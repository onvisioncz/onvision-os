...
async function isAdmin(supabase: Awaited<ReturnType<typeof createClient>>, email: string): Promise<boolean> {
  try {
...
export async function POST(req: NextRequest) {
  ...
  if (!(await isAdmin(supabase, user.email))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  const { data: trashData } = await supabase.from("app_data").select("value").eq("key", "ov-trash").maybeSingle();
  ...
  const { data: srcData } = await supabase.from("app_data").select("value").eq("key", entry.srcKey).maybeSingle();
